...
#' @noRd
#' 
createUmapSave <- function(df, grey_df = NULL){
...
createUmap <- function(df, highlight_df = NULL, grey_df = NULL){
  ...
  # colour functions - should make this a function ----
  topics <- unique(df$kmeans_topic_title)
  colours <- viridis::viridis(n = length(topics), begin = 0, end = 0.92, option = "D", direction = 1)
  # names(colours) <- topics
  colour_lighter <- adjust_colour_lighter(colours, og_val = 0.6)
  names(colour_lighter) <- topics
  # colour_darker <- adjust_colour_darker(colours, og_val = 5)
  # names(colour_darker) <- topics
  # ----
  
  # cluster labelling and colouring - should make this a function ----
  ...
  p <- plotly::plot_ly(
    ...
    plotly::add_trace(data = plot_df,
                      ...
                      hoverlabel = list(bgcolor = 'rgba(255,255,255,0.75)',
                                        ...
                                        # color = ~colours[kmeans_topic_title]
                                        ),
                      showlegend = TRUE,
  ...
  p <- p %>%
    ...
    plotly::add_annotations(
      ...
      # text = sprintf("<b>%s</b>", cluster_lookup$label),
  ...
  for (i in 1:nrow(cluster_lookup)) {
    ...
    # formatted_label <- sprintf("<b>%s</b>", cluster_lookup$label[i]) # bold
    ...
    p <- p %>% plotly::add_annotations(
      ...
      font = list(size = 22, family = "Cinzel", 
                  # color = colours[cluster_lookup$label[i]])
